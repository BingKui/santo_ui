...
class SantoDeleteTag extends StatefulWidget {
  ...
  SantoDeleteTag(
      ...
      this.themeData})
      : super(key: key) {
    themeData ??= SantoTagConfig();
    themeData = SantoThemeConfigurator.instance
        .getConfig(configId: themeData!.configId)
        .tagConfig
        .merge(this.themeData);
    themeData = themeData!.merge(SantoTagConfig(
        tagBackgroundColor: this.backgroundColor,
        tagTextStyle: SantoTextStyle.withStyle(tagTextStyle)));
  }

  @override
  _SantoDeleteTagState createState() => _SantoDeleteTagState();
}

class _SantoDeleteTagState extends State<SantoDeleteTag> {
  late SantoDeleteTagController _controller;

  @override
  void initState() {
    super.initState();
    _controller =
        widget.controller ?? SantoDeleteTagController(initTags: widget.tags);
  }
...
/// 标签控制器，用于主动添加和删除标签
class SantoDeleteTagController {
  ...
  SantoDeleteTagController({List<String>? initTags}) {
    _tags = initTags ?? [];
