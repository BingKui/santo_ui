...
class SantoTextSelectFormItem extends StatefulWidget {
  ...
  @override
  SantoTextSelectFormItemState createState() {
    return SantoTextSelectFormItemState();
  }
}

double _fontSize = SantoFonts.f16;
StrutStyle _contentStructStyle = StrutStyle(
    forceStrutHeight: true, height: 1, leading: 0.5, fontSize: _fontSize);

class SantoTextSelectFormItemState extends State<SantoTextSelectFormItem> {
  @override
  ...
  double _getTitleMaxWidth(BuildContext context, BoxConstraints constraints) {
    double contentRatio = SantoFormUtil.getAutoLayoutContentRatio(
    ...
    double maxWidth = min(
        ...
        _calculateTextWidth(context) +
            SantoFormUtil.rightArrowLeftPadding +
            SantoFormUtil.rightArrowSize);
    return maxWidth;
  }
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: SantoFormUtil.getRightArrowIcon(),
            ),
          ],
        ),
      ),
            ...
            Container(
              ...
              child: Text(
                ...
                strutStyle: StrutStyle(
                    ...
                    height: 1,
                    leading: 0.5,
                    fontSize: SantoFonts.f14),
                style: SantoFormUtil.getTipsTextStyle(widget.themeData!),
              ),
            ),
          ],
        ),
  ...
  Widget _buildTitleTextWidget() {
    return Text(
      ...
      style: SantoFormUtil.getTitleTextStyle(widget.themeData!, height: 1),
    );
  }
    ...
    if (widget.value != null && widget.value!.isNotEmpty) {
      ...
      painter = TextPainter(
          ...
          text: TextSpan(
            ...
            style: SantoFormUtil.getHintTextStyle(widget.themeData!, height: 1),
          ));
    }
    painter.layout();
  ...
  Widget buildText() {
    if (widget.value != null && widget.value!.isNotEmpty) {
      ...
      return Text(
        widget.hint ?? SantoIntl.of(context).localizedResource.pleaseChoose,
        textAlign: TextAlign.end,
        strutStyle: _contentStructStyle,
        style: SantoFormUtil.getHintTextStyle(widget.themeData!, height: 1),
      );
  ...
  String getCalculateText() {
    String value = SantoIntl.of(context).localizedResource.pleaseChoose;
    if (!SantoTools.isEmpty(widget.value)) {
      value = widget.value!;
    } else if (!SantoTools.isEmpty(widget.hint)) {
      value = widget.hint ?? SantoIntl.of(context).localizedResource.pleaseChoose;
    }
    return value;
  }
