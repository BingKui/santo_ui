...
class SantoSelectionEntity {
  ...
  late SantoSelectionFilterType filterType;
  ...
  /// 建议使用 [SantoSelectionEntity.fromJson]
  static SantoSelectionEntity fromMap(Map<String, dynamic> map) {
    SantoSelectionEntity entity = SantoSelectionEntity();
    ...
    if (map['children'] != null && map['children'] is List) {
      entity.children = []..addAll(
          (map['children'] as List).map((o) => SantoSelectionEntity.fromMap(o)));
    }
    entity.filterType = entity.parserFilterTypeWithType(map['type'] ?? "");
    return entity;
  }

  SantoSelectionEntity.fromJson(Map<dynamic, dynamic>? map)
      : this.title = '',
        this.maxSelectedCount = SantoSelectionConstant.maxSelectCount,
        this.isCustomTitleHighLight = false,
        this.isSelected = false,
    ...
    extMap = map['ext'] ?? {};
    children = []..addAll(
        (map['children'] ?? []).map((o) => SantoSelectionEntity.fromJson(o)));
  ...
  void configRelationship() {
    if (children.isNotEmpty) {
      for (SantoSelectionEntity entity in children) {
        entity.parent = this;
      }
      for (SantoSelectionEntity entity in children) {
  ...
  void configDefaultValue() {
    if (children.isNotEmpty) {
      ...
      if (children.where((item) => item.isSelected).toList().isEmpty) {
        List<SantoSelectionEntity> rangeItems = this.children.where((item) {
          return (item.filterType == SantoSelectionFilterType.range ||
              item.filterType == SantoSelectionFilterType.dateRange ||
              item.filterType == SantoSelectionFilterType.dateRangeCalendar);
        }).toList();
        SantoSelectionEntity? rangeEntity;
        if (rangeItems.isNotEmpty) {
          rangeEntity = rangeItems[0];
        }
        if (rangeEntity != null && !SantoTools.isEmpty(defaultValue)) {
          List<String> values = defaultValue!.split(':');
          if (values.length == 2 &&
      ...
      for (SantoSelectionEntity entity in this.children) {
        entity.configDefaultValue();
      }
      if (hasCheckBoxBrother()) {
        isSelected = children.where((item) => item.isSelected).isNotEmpty;
      } else {
        isSelected =
            isSelected || children.where((item) => item.isSelected).isNotEmpty;
      }
  ...
  /// 根据 [showType] 解析出对应的 [SantoSelectionWindowType]， 默认为 [SantoSelectionWindowType.list]
  SantoSelectionWindowType parserShowType(String? showType) {
    if (showType == "list") {
      return SantoSelectionWindowType.list;
  ...
  /// 根据 [type] 解析出对应的 [SantoSelectionFilterType], 默认为 [SantoSelectionFilterType.none]
  SantoSelectionFilterType parserFilterTypeWithType(String? type) {
    if (type == 'unlimit') {
      return SantoSelectionFilterType.unLimit;
    } else if (type == "radio") {
      return SantoSelectionFilterType.radio;
    } else if (type == "checkbox") {
      return SantoSelectionFilterType.checkbox;
    } else if (type == "range") {
      return SantoSelectionFilterType.range;
    } else if (type == "customHandle") {
      return SantoSelectionFilterType.customHandle;
    } else if (type == "more") {
      return SantoSelectionFilterType.more;
    } else if (type == 'floatinglayer') {
      return SantoSelectionFilterType.layer;
    } else if (type == 'customfloatinglayer') {
      return SantoSelectionFilterType.customLayer;
    } else if (type == 'date') {
      return SantoSelectionFilterType.date;
    } else if (type == 'daterange') {
      return SantoSelectionFilterType.dateRange;
    } else if (type == 'daterangecalendar') {
      return SantoSelectionFilterType.dateRangeCalendar;
    }
    return SantoSelectionFilterType.none;
  }
  ...
  void clearChildSelection() {
    if (children.isNotEmpty) {
      for (SantoSelectionEntity entity in children) {
        ...
        if (entity.filterType == SantoSelectionFilterType.range ||
            entity.filterType == SantoSelectionFilterType.dateRange ||
            entity.filterType == SantoSelectionFilterType.dateRangeCalendar) {
  ...
  /// 获取当前节点的所有选中的子节点
  List<SantoSelectionEntity> selectedLastColumnList() {
    List<SantoSelectionEntity> list = [];
    if (this.children.isNotEmpty) {
      List<SantoSelectionEntity> firstList = [];
      for (SantoSelectionEntity firstEntity in this.children) {
        if (firstEntity.children.isNotEmpty) {
          List<SantoSelectionEntity> secondList = [];
          for (SantoSelectionEntity secondEntity in firstEntity.children) {
            if (secondEntity.children.isNotEmpty) {
              List<SantoSelectionEntity> thirds =
  ...
  List<SantoSelectionEntity> selectedListWithoutUnlimit() {
    List<SantoSelectionEntity> selected = selectedList();
    return selected
        .where((item) => !item.isUnLimit())
        .where((item) =>
            (item.filterType != SantoSelectionFilterType.range) ||
            (item.filterType == SantoSelectionFilterType.range &&
                !SantoTools.isEmpty(item.customMap)))
        .where((item) =>
            (item.filterType != SantoSelectionFilterType.dateRange) ||
            (item.filterType == SantoSelectionFilterType.dateRange &&
                !SantoTools.isEmpty(item.customMap)))
  ...
  List<SantoSelectionEntity> selectedList() {
    if (SantoSelectionFilterType.more == this.filterType) {
      return this.selectedLastColumnList();
    } else {
      List<SantoSelectionEntity> results = [];
      List<SantoSelectionEntity> firstColumn =
          SantoSelectionUtil.currentSelectListForEntity(this);
  ...
  /// 获取当前节点的所有选中的子节点
  List<SantoSelectionEntity> allSelectedList() {
    List<SantoSelectionEntity> results = [];
    List<SantoSelectionEntity> firstColumn =
        SantoSelectionUtil.currentSelectListForEntity(this);
    results.addAll(firstColumn);
  ...
  /// 获取当前节点对应根节点
  SantoSelectionEntity getRootEntity(SantoSelectionEntity rootEntity) {
  ...
  int getSelectedChildCount(SantoSelectionEntity entity) {
    if (SantoTools.isEmpty(entity.children)) {
  ...
  int getIndexInCurrentLevel() {
    ...
    for (SantoSelectionEntity entity in parent!.children) {
      if (entity == this) {
  ...
  bool hasCheckBoxBrother() {
    int? count = parent?.children
        .where((f) => f.filterType == SantoSelectionFilterType.checkbox)
        .length;
  ...
  bool isUnLimit() {
    return filterType == SantoSelectionFilterType.unLimit;
  }

  /// 递归清空选中状态
  void clearSelectedEntity() {
    List<SantoSelectionEntity> tmp = [];
    SantoSelectionEntity node = this;
    tmp.add(node);
  ...
  List<SantoSelectionEntity> currentTagListForEntity() {
    List<SantoSelectionEntity> list = [];
    children.forEach((data) {
      if (data.filterType != SantoSelectionFilterType.range &&
          data.filterType != SantoSelectionFilterType.dateRange &&
          data.filterType != SantoSelectionFilterType.dateRangeCalendar) {
        list.add(data);
      }
    });
    return list;
  ...
  /// 根据是否展开，返回对应数量的子节点
  List<SantoSelectionEntity> currentShowTagByExpanded(bool isExpanded) {
    List<SantoSelectionEntity> all = currentTagListForEntity();
  ...
  /// [SantoSelectionFilterType.dateRange]
  /// [SantoSelectionFilterType.dateRangeCalendar]
  List<SantoSelectionEntity> currentRangeListForEntity() {
    List<SantoSelectionEntity> list = [];
    children.forEach((data) {
      if (data.filterType == SantoSelectionFilterType.range ||
          data.filterType == SantoSelectionFilterType.dateRange ||
  ...
  /// 校验 range 类型的数据是否合法
  bool isValidRange() {
    if (this.filterType == SantoSelectionFilterType.range ||
        this.filterType == SantoSelectionFilterType.dateRange ||
        this.filterType == SantoSelectionFilterType.dateRangeCalendar) {
