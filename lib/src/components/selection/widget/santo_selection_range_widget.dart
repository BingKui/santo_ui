...
import 'package:santo_ui/src/components/selection/widget/santo_selection_range_input_item_widget.dart';
...
/// 范围选择子组件
class SantoRangeSelectionGroupWidget extends StatefulWidget {
  /// 根节点的筛选数据
  ...
  /// 确认按钮点击事件
  final SantoOnRangeSelectionConfirm? onSelectionConfirm;
  ...
  SantoRangeSelectionGroupWidget(
      {Key? key,
  ...
  @override
  _SantoRangeSelectionGroupWidgetState createState() =>
      _SantoRangeSelectionGroupWidgetState();
}

class _SantoRangeSelectionGroupWidgetState
    extends State<SantoRangeSelectionGroupWidget>
    with SingleTickerProviderStateMixin {
  List<SantoSelectionEntity> _originalSelectedItemsList = [];
  List<SantoSelectionEntity> _firstList = [];
  List<SantoSelectionEntity> _secondList = [];
  int _firstIndex = -1;
  int _secondIndex = -1;
  int totalLevel = 0;
  ...
  Widget build(BuildContext context) {
    totalLevel = SantoSelectionUtil.getTotalLevel(widget.entity);
  ...
  Widget _createNewTagAndRangeWidget(
    ...
    if (firstList.isNotEmpty &&
      ...
      return Column(
        ...
        SantoSelectionUtil.getTotalLevel(widget.entity) == 2) {
      var tabBar = SantoTabBar(
        tabHeight: 50,
  ...
  List<Widget> _getOneTabContent(SantoSelectionEntity filterItem) {
    ...
    List<SantoSelectionEntity> tagFilterList = subFilterList
        .where((f) =>
            f.filterType != SantoSelectionFilterType.range &&
            f.filterType != SantoSelectionFilterType.date &&
            f.filterType != SantoSelectionFilterType.dateRange &&
            f.filterType != SantoSelectionFilterType.dateRangeCalendar)
        .toList();
    Size maxWidthSize = Size.zero;
    for (SantoSelectionEntity entity in subFilterList) {
      Size size = SantoTextUtil.textSize(entity.title,
    ...
    Widget? content;
    for (SantoSelectionEntity item in subFilterList) {
      if (item.filterType == SantoSelectionFilterType.range) {
        content = SantoSelectionRangeItemWidget(
            item: item,
        ...
        break;
      } else if (item.filterType == SantoSelectionFilterType.dateRange) {
        content = SantoSelectionDateRangeItemWidget(
            item: item,
            minTextEditingController: _minTextEditingController,
            maxTextEditingController: _maxTextEditingController,
            themeData: widget.themeData,
        ...
        break;
      } else if (item.filterType == SantoSelectionFilterType.date) {
      ...
      } else if (item.filterType == SantoSelectionFilterType.dateRangeCalendar) {
  ...
  void _confirmButtonClickEvent() {
    ...
    if (totalLevel == 2) {
      List<SantoSelectionEntity> subFilterList =
          widget.entity.children[_tabController.index].children;
      List<SantoSelectionEntity> selectItems =
    ...
    // 处理Range类型的校验
    SantoSelectionEntity? rangeEntity = _getSelectRangeItem(totalLevel == 1
        ? _firstList
        : _firstList[_tabController.index].children);
    if (rangeEntity != null) {
      if (rangeEntity.customMap != null &&
          (!SantoTools.isEmpty(rangeEntity.customMap!['min']) ||
              !SantoTools.isEmpty(rangeEntity.customMap!['max']))) {
        if (!rangeEntity.isValidRange()) {
          FocusScope.of(context).requestFocus(FocusNode());
          if (rangeEntity.filterType == SantoSelectionFilterType.range) {
            ...
            SantoToast.show(SantoIntl.of(context).localizedResource.selectRangeError, context);
          }
          return;
        }
      } else {
  ...
  void _setFirstIndex(int firstIndex) {
    ...
    if (widget.entity.children.length > _firstIndex) {
      List<SantoSelectionEntity> seconds =
          widget.entity.children[_firstIndex].children;
      if (seconds.isNotEmpty) {
        for (SantoSelectionEntity entity in seconds) {
  ...
  void _configDefaultSelectedData() {
    ...
    for (SantoSelectionEntity entity in _firstList) {
      if (entity.isSelected) {
    ...
    if (_firstIndex >= 0 && _firstIndex < _firstList.length) {
      ...
      if (_secondList.isNotEmpty) {
        for (SantoSelectionEntity entity in _secondList) {
          if (entity.isSelected) {
  ...
  /// 设置数据为未选中状态
  void _resetSelectionDatas(SantoSelectionEntity entity) {
  ...
  void _clearNotTagItem(List<SantoSelectionEntity> subFilterList) {
    subFilterList
        .where((f) =>
            f.filterType == SantoSelectionFilterType.range ||
            f.filterType == SantoSelectionFilterType.date ||
  ...
  void _clearTagSelectStatus(List<SantoSelectionEntity> subFilterList) {
    subFilterList
        .where((f) => f.filterType != SantoSelectionFilterType.range)
        .where((f) => f.filterType != SantoSelectionFilterType.date)
        .where((f) => f.filterType != SantoSelectionFilterType.dateRange)
        .where((f) => f.filterType != SantoSelectionFilterType.dateRangeCalendar)
        .forEach((f) {
      f.isSelected = false;
      f.customMap = Map();
    });
  ...
  SantoSelectionEntity? _getSelectRangeItem(List<SantoSelectionEntity> filterList) {
    List<SantoSelectionEntity> ranges = filterList
        .where((f) =>
            (f.filterType == SantoSelectionFilterType.range ||
                f.filterType == SantoSelectionFilterType.dateRange ||
                f.filterType == SantoSelectionFilterType.dateRangeCalendar) &&
  ...
  void _resetCustomMapData() {
    for (SantoSelectionEntity commonEntity in _originalSelectedItemsList) {
  ...
  int _getInitFocusedIndex(List<SantoSelectionEntity> subFilterList) {
    ...
    var selectedItem = subFilterList
        .where((f) =>
            ...
            f.isSelected)
        .toList();
    if (!isCustomInputSelected && SantoTools.isEmpty(selectedItem)) {
      for (SantoSelectionEntity item in subFilterList) {
        if (item.isUnLimit()) {
  ...
  bool _hasCalendarItem(SantoSelectionEntity entity) {
    bool hasCalendarItem = false;
    hasCalendarItem = entity.children
            .where((item) =>
                item.filterType == SantoSelectionFilterType.date ||
                item.filterType == SantoSelectionFilterType.dateRangeCalendar)
            .toList().isNotEmpty;

    /// 查找第二层级
    if (!hasCalendarItem) {
      for (SantoSelectionEntity subItem in entity.children) {
