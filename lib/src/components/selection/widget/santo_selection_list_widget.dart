...
import 'package:santo_ui/src/components/selection/widget/santo_selection_single_list_widget.dart';
...
typedef SingleListItemSelect = void Function(
    int listIndex, int index, SantoSelectionEntity entity);

/// 多列数据选择子组件，内部包含多个 SantoSelectionSingleListWidget
// ignore: must_be_immutable
class SantoListSelectionGroupWidget extends StatefulWidget {
...
class _SantoSelectionGroupViewState extends State<SantoListSelectionGroupWidget> {
  final int maxShowCount = 6;

  List<SantoSelectionEntity> _firstList = [];
  List<SantoSelectionEntity> _secondList = [];
  List<SantoSelectionEntity> _thirdList = [];
  ...
  List<Widget> _configWidgets() {
    ...
    // TODO 判断是否添加 Bottom
    if (totalLevel == 1 &&
        widget.entity.filterType == SantoSelectionFilterType.radio) {
    } else {
  ...
  Widget _listWidget() {
    ...
    if (!SantoTools.isEmpty(_firstList) &&
      ...
      /// 1、仅有一级的情况
      /// 1.1 一级单选 && 没有自定义范围的情况
      widgets.add(SantoSelectionSingleListWidget(
          items: _firstList,
  ...
  ///还原数据选中状态
  void _resetSelectionData(SantoSelectionEntity entity) {
  ...
  void _setSecondIndex(int secondIndex) {
    ...
    if (seconds.length > _secondIndex) {
      List<SantoSelectionEntity> thirds = seconds[_secondIndex].children;
  ...
  int _getInitialSelectIndex(List<SantoSelectionEntity> levelList) {
    int index = -1;
    if (levelList.isEmpty) {
      return index;
    }

    for (SantoSelectionEntity entity in levelList) {
      if (entity.isSelected) {
    ...
    if (index < 0) {
      for (SantoSelectionEntity entity in levelList) {
        if (entity.isUnLimit() &&
            SantoSelectionUtil.getTotalLevel(entity) > 1 &&
  ...
  _processSelectedStatus(SantoSelectionEntity entity) {
    if (entity.children.isNotEmpty) {
      entity.children.forEach((f) => _processSelectedStatus(f));
      if (entity.hasCheckBoxBrother()) {
        entity.isSelected =
            entity.children.where((item) => item.isSelected).isNotEmpty;
      }
    }
  }
