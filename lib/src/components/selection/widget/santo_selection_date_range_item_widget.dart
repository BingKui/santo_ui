...
import 'package:santo_ui/src/components/picker/time_picker/date_picker/santo_date_widget.dart';
import 'package:santo_ui/src/components/selection/bean/santo_selection_common_entity.dart';
import 'package:santo_ui/src/components/selection/controller/santo_selection_view_date_picker_controller.dart';
import 'package:santo_ui/src/components/selection/widget/santo_selection_datepicker_animate_widget.dart';
...
class SantoSelectionDateRangeItemWidget extends StatefulWidget {
  ...
  SantoSelectionDateRangeItemWidget(
      {Key? key,
      required this.item,
  ...
  _SantoSelectionDateRangeItemWidgetState createState() =>
      _SantoSelectionDateRangeItemWidgetState();
}

class _SantoSelectionDateRangeItemWidgetState
    extends State<SantoSelectionDateRangeItemWidget> {
  SantoSelectionDatePickerController _datePickerController =
          ...
          children: <Widget>[
            widget.isNeedTitle
                ? Container(
                    ...
                    child: Text(
                      widget.item.title.isEmpty ? SantoIntl.of(context).localizedResource.customRange : widget.item.title,
  ...
  void onTextTapped(bool isMax) {
    ...
    DateTime? minSelectedDateTime = SantoTools.isEmpty(widget.item.customMap)
        ? null
        : DateTimeFormatter.convertIntValueToDateTime(
            widget.item.customMap!['min']);
    DateTime? maxSelectedDateTime = SantoTools.isEmpty(widget.item.customMap)
        ? null
        : DateTimeFormatter.convertIntValueToDateTime(
            widget.item.customMap!['max']);
    ...
    Widget content = SantoDateWidget(
      canPop: false,
      ...
      pickerTitleConfig: SantoPickerTitleConfig(
          ...
          titleContent: isMax ? SantoIntl.of(context).localizedResource.selectEndDate : SantoIntl.of(context).localizedResource.selectStartDate),
      onCancel: () {
      ...
      onConfirm: (DateTime selectedDate, List<int> selectedIndex) {
        ...
        if (!isMax &&
            SantoTools.isEmpty(widget.maxTextEditingController.text)) {
          onTextTapped(true);
        }
        setState(() {});
      },
