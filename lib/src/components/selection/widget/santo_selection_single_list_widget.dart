...
import 'package:santo_ui/src/components/selection/widget/santo_selection_common_item_widget.dart';
...
class SantoSelectionSingleListWidget extends StatefulWidget {
  late List<SantoSelectionEntity> _selectedItems;
  ...
  SantoSelectionSingleListWidget({
    Key? key,
    required this.items,
    required this.flex,
    this.focusedIndex = -1,
    ...
    /// 当前 Items 所在的层级
    currentListIndex = SantoSelectionUtil.getCurrentListIndex(
        items.isNotEmpty ? items[0] : null);
    _selectedItems = items.where((f) => f.isSelected).toList();
  }

  @override
  _SantoSelectionSingleListWidgetState createState() =>
      _SantoSelectionSingleListWidgetState();
}

class _SantoSelectionSingleListWidgetState
    extends State<SantoSelectionSingleListWidget> {
  @override
      ...
      child: Container(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (BuildContext context, int index) {
            SantoSelectionEntity item = widget.items[index];

            /// 点击筛选，展开弹窗时，默认展示上次选中的筛选项。
            bool isCurrentFocused = isItemFocused(index, item);

            return SantoSelectionCommonItemWidget(
              item: item,
              ...
              itemSelectFunction: (SantoSelectionEntity entity) {
                if ((entity.filterType == SantoSelectionFilterType.checkbox &&
                        !entity.isSelected) ||
                  ...
                  if (entity.hasCheckBoxBrother()) {
                    if (entity.isUnLimit() &&
                      ...
                      /// 同级别中，存在不限类型已经选中情况，选择非不限类型 item，不检查数量限制
                        ...
                        !SantoSelectionUtil.checkMaxSelectionCount(entity)) {
                      SantoToast.show(SantoIntl.of(context).localizedResource.filterConditionCountLimited, context);
                      return;
                    }
                  } else {
                    if (!SantoSelectionUtil.checkMaxSelectionCount(entity)) {
                      SantoToast.show(SantoIntl.of(context).localizedResource.filterConditionCountLimited, context);
                      return;
                    }
                  }
  ...
  bool isItemFocused(int itemIndex, SantoSelectionEntity item) {
  ...
  /// Item 点击之后的数据处理
  void _processFilterData(SantoSelectionEntity selectedEntity) {
    int totalLevel = SantoSelectionUtil.getTotalLevel(selectedEntity);
    ...
    for (SantoSelectionEntity item in widget.items) {
      if (item.isSelected) {
  ...
  void configOneLevelList(SantoSelectionEntity selectedEntity) {
    if (SantoSelectionFilterType.radio == selectedEntity.filterType) {
      /// 单选，清除同一级别选中的状态，则其他的设置为未选中。
      selectedEntity.parent?.clearChildSelection();
      selectedEntity.isSelected = true;
    } else if (SantoSelectionFilterType.checkbox == selectedEntity.filterType) {
      /// 选中【不限】清除同一级别其他的状态
      if (selectedEntity.isUnLimit()) {
        selectedEntity.parent?.clearChildSelection();
        selectedEntity.isSelected = true;
  ...
  void configMultiLevelList(
      SantoSelectionEntity selectedEntity, int currentListIndex) {
    /// 选中【不限】清除同一级别其他的状态
    if(selectedEntity.isUnLimit()){
      ...
      selectedEntity.isSelected = true;
    } else if (SantoSelectionFilterType.radio == selectedEntity.filterType) {
      /// 单选，清除同一级别选中的状态，则其他的设置为未选中。
      selectedEntity.parent?.children
          .where((f) => f != selectedEntity)
          .forEach((f) {
