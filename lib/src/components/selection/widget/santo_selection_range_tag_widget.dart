...
/// 描述: 多选 tag 组件
/// /// /// /// /// /// /// /// /// /
class SantoSelectionRangeTagWidget extends StatefulWidget {
  /// tag 显示的文本
  @required
  final List<SantoSelectionEntity> tagFilterList;
  ...
  @override
  _SantoSelectionRangeTagWidgetState createState() =>
      _SantoSelectionRangeTagWidgetState();
}

class _SantoSelectionRangeTagWidgetState
    extends State<SantoSelectionRangeTagWidget> {
  @override
    ...
    for (int nameIndex = 0;
      ...
      GestureDetector gdt = GestureDetector(
          ...
          onTap: () {
            ...
            if (SantoSelectionFilterType.checkbox == selectedEntity.filterType &&
                !selectedEntity.isSelected) {
              if (!SantoSelectionUtil.checkMaxSelectionCount(selectedEntity)) {
  ...
  Widget _tagWidgetAtIndex(int nameIndex) {
    ...
    if (widget.tagFilterList[nameIndex].filterType ==
            SantoSelectionFilterType.date &&
        !SantoTools.isEmpty(widget.tagFilterList[nameIndex].value)) {
      if (int.tryParse(widget.tagFilterList[nameIndex].value ?? '') != null) {
        ...
        if (dateTime != null) {
          text = DateTimeFormatter.formatDate(
              dateTime, SantoIntl.of(context).localizedResource.dateFormatYYYYMMDD);
        }
      } else {
