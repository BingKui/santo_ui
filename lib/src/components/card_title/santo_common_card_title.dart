...
///    subTitleWidget: SantoStartRatingIndicator(rating: 4),
///    onTap: () {
...
/// )
/// 相关按钮如下:
///  * [SantoActionCardTitle], 右侧为箭头的卡片标题组件
///
class SantoCommonCardTitle extends StatelessWidget {
  /// 标题
  final String title;
  ...
  /// the theme config of SantoCommonCardTitle
  final SantoCardTitleConfig? themeData;

  /// create SantoCommonCardTitle
  const SantoCommonCardTitle(
      {Key? key,
      required this.title,
      this.accessoryText,
  ...
  Widget build(BuildContext context) {
    ...
    SantoCardTitleConfig cardTitleConfig = SantoCardTitleConfig(
        alignment: alignment,
        cardTitlePadding: padding as EdgeInsets?,
        detailTextStyle: SantoTextStyle(color: detailColor));
  ...
  ///标题widget
  Widget _titleWidget(BuildContext context, SantoCardTitleConfig defaultConfig) {
