import 'package:santo_ui/src/components/charts/santo_progress_bar_chart/santo_progress_bar_chart_painter.dart';
import 'package:flutter/material.dart';

/// SantoProgressBarChart
/// 柱形数据图表
/// 展示一组或者多组数据，方便数据的查看以及对比
// ignore: must_be_immutable
class SantoProgressBarChart extends StatefulWidget {
  /// 图表最小宽度，默认0
  ...
  /// 柱形数据，必传
  final List<SantoProgressBarBundle> barBundleList;
  ...
  final SantoProgressBarChartSelectCallback? barChartSelectCallback;

  /// 图表高度，竖直柱状图有效，默认300
  final double height;

  /// create SantoProgressBarChart
  SantoProgressBarChart(
      {Key? key,
      this.minWidth = 0,
  ...
  State<StatefulWidget> createState() {
    return SantoProgressBarChartState();
  }
}

class SantoProgressBarChartState extends State<SantoProgressBarChart> {
  SantoProgressBarItem? _selectedBarItem;
  ...
  Size _chartSize() {
    ...
    if (BarChartStyle.horizontal == widget.barChartStyle) {
      ...
      if (widget.yAxis.axisItemList.isNotEmpty) {
        width += SantoProgressBarChartPainter.maxYAxisWidth(widget.yAxis);
      }
      return Size(
  ...
  @override
  void didUpdateWidget(SantoProgressBarChart oldWidget) {
    super.didUpdateWidget(oldWidget);
    this._clearSelectedBarItem(oldWidget);
  }

  void _clearSelectedBarItem(SantoProgressBarChart oldWidget) {
    if (widget.barBundleList.length == oldWidget.barBundleList.length) {
      ...
      for (int bundleIndex = 0; bundleIndex < bundleCount; bundleIndex++) {
        SantoProgressBarBundle bundle = widget.barBundleList[bundleIndex];
        SantoProgressBarBundle oldBundle = oldWidget.barBundleList[bundleIndex];
        if (bundle.barList.length == oldBundle.barList.length) {
          ...
          for (int barIndex = 0; barIndex < barCount; barIndex++) {
            SantoProgressBarItem barItem = bundle.barList[barIndex];
            SantoProgressBarItem oldBarItem = oldBundle.barList[barIndex];
  ...
  Widget build(BuildContext context) {
    ...
    if (BarChartStyle.vertical == widget.barChartStyle) {
      double yAxisWidth =
          SantoProgressBarChartPainter.maxYAxisWidth(widget.yAxis);
      return Padding(
        ...
        child: Stack(
          children: <Widget>[
            CustomPaint(
              size: chartSize,
              painter: SantoProgressBarChartPainter(
                drawBar: false,
                drawX: false,
                xAxis: widget.xAxis,
                yAxis: widget.yAxis,
              ...
              child: SingleChildScrollView(
                ...
                child: CustomPaint(
                  ...
                  painter: SantoProgressBarChartPainter(
                      ...
                      santoProgressBarChartSelectCallback:
                          (SantoProgressBarItem? item) {
              ...
              child: SingleChildScrollView(
                ...
                child: CustomPaint(
                  ...
                  painter: SantoProgressBarChartPainter(
                      ...
                      selectedHintTextBackgroundColor:
                      widget.selectedHintTextBackgroundColor,
                      santoProgressBarChartSelectCallback: (SantoProgressBarItem? item) {
