...
///   每层layer支持自定义图案绘制，绘制参考[SantoFunnelLayerPainter]类定义
class SantoFunnelChart extends MultiChildRenderObjectWidget {
  ///漏斗的最大宽度，一般是漏斗最上层的宽度
  ...
  ///漏斗图默认Santo风格的命名构造函数，[layerCount]不能大于[defaultLayerColors.length]。
  SantoFunnelChart.defaultStyle({
    Key? key,
    required this.layerCount,
    required this.markerCount,
    required MarkerBuilder builder,
    ...
    this.layerMargin = 0,
    this.childOffset = Offset.zero,
  })  : this.layerPainter = SantoDefaultFunnelLayerPainter(),
        this.shape = FunnelShape.leftAndRight,
        this.alignment = MarkerAlignment.right,
...
class SantoFunnelChartParentData extends ContainerBoxParentData<RenderBox> {}

abstract class RenderFunnelChart extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, SantoFunnelChartParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, SantoFunnelChartParentData> {
  RenderFunnelChart({
    ...
    required SantoFunnelLayerPainter layerPainter,
  ...
  SantoFunnelLayerPainter get layerPainter => _layerPainter;

  SantoFunnelLayerPainter _layerPainter;

  set layerPainter(SantoFunnelLayerPainter value) {
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! SantoFunnelChartParentData) {
      child.parentData = SantoFunnelChartParentData();
    }
  }
...
///漏斗图RenderObject
class SantoFunnelRender extends RenderFunnelChart {
  ...
  double? getIntrinsicDimensionHorizontal(
    ...
    if (_alignment == MarkerAlignment.center) {
      while (child != null) {
        final SantoFunnelChartParentData childParentData =
            child.parentData as SantoFunnelChartParentData;
      ...
      while (child != null) {
        ...
        num++;
        SantoFunnelChartParentData childParentData =
            child.parentData as SantoFunnelChartParentData;
        child = childParentData.nextSibling;
      }
    }
...
///漏斗每层绘制接口类，自定义每层绘制继承该类。示例查看[SantoDefaultFunnelLayerPainter]。
abstract class SantoFunnelLayerPainter {
  ///该层layer的颜色是否渐变
