...
/// 选中扇形区域后执行的回调
typedef SantoDoughnutSelectCallback = void Function(
    SantoDoughnutDataItem? selectedItem);

class SantoDoughnut extends CustomPainter {
  ///圆心位置
  late Offset circleCenter;

  /// 选中的区域
  final SantoDoughnutDataItem? selectedItem;

  /// 选中区域回调
  final SantoDoughnutSelectCallback? santoDoughnutSelectCallback;
...
/// SantoDoughnutChart 圆形数据展示组件
/// 可选择使用圆形、环形来展示数据所占总数的百分比
class SantoDoughnutChart extends StatelessWidget {
  /// 宽度。默认值 0
  final double width;
  ...
  /// 选中项目时候的回掉
  final SantoDoughnutSelectCallback? selectCallback;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: CustomPaint(
        ...
        foregroundPainter: SantoDoughnut(
            data: this.data,
            ...
            santoDoughnutSelectCallback: this.selectCallback),
      ),
    );
  }
}
