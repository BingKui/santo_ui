...
///对绘制的效果也支持一定程度上的定制，参考[SantoRadarChartDataProvider],[SantoRadarChartStyle]。
class SantoRadarChart extends MultiChildRenderObjectWidget {
  ...
  ///提供绘制雷达图所需要的数据
  final SantoRadarChartDataProvider provider;
...
class SantoRadarChartParentData extends ContainerBoxParentData<RenderBox> {}
...
class RenderRadarChart extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, SantoRadarChartParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, SantoRadarChartParentData> {
  Paint _radarPainter;
  Paint _axisPainter;
  ...
  SantoRadarChartDataProvider _dataProvider;
  ...
  RenderRadarChart({
    ...
    required SantoRadarChartDataProvider provider,
  ...
  SantoRadarChartDataProvider get dataProvider => _dataProvider;

  set dataProvider(SantoRadarChartDataProvider value) {
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! SantoRadarChartParentData) {
      child.parentData = SantoRadarChartParentData();
    }
  }
  ...
  double _getIntrinsicDimensionHorizontal(
    ...
    int i = 0;
    while (child != null) {
      final SantoRadarChartParentData childParentData =
          child.parentData as SantoRadarChartParentData;
  ...
  void _drawRadar(Canvas canvas) {
    ...
    for (int radarIndex = 0; radarIndex < radarCount; radarIndex++) {
      SantoRadarChartStyle radarStyle = _dataProvider.getRadarStyle(radarIndex);
      _radarPainter
        ..isAntiAlias = true
...
abstract class SantoRadarChartDataProvider {
  ...
  ///The values determine radar's vertexes.The length must be same as [SantoRadarChart.sidesCount].
  ///The value must range from [SantoRadarChart.minValue] to [SantoRadarChart.maxValue].
...
///The default SantoRadarChartDataProvider which using the default designed style.
class DefaultRadarProvider extends SantoRadarChartDataProvider {
