...
import 'package:santo_ui/src/components/charts/broken_line/santo_base_painter.dart';
import 'package:santo_ui/src/components/charts/broken_line/santo_line_data.dart';
import 'package:santo_ui/src/components/charts/broken_line/monotone_x.dart';
import 'package:santo_ui/src/theme/santo_theme_configurator.dart';
import 'package:flutter/material.dart';
import 'package:path_drawing/path_drawing.dart';

/// 折线图 刻度线、X/Y 轴绘制类
class SantoLinePainter extends SantoBasePainter {
  ...
  /// x轴 刻度
  List<SantoDialItem>? xDialValues;

  /// y轴左侧刻度显示，不传则没有
  List<SantoDialItem>? yDialValues;
  ...
  @override
  bool shouldRepaint(SantoLinePainter oldDelegate) {
    return true;
  }
  ...
  void _initValue() {
    ...
    yDialColor ??= SantoThemeConfigurator.instance
        .getConfig()
        .commonConfig
        .colorTextSecondary;
    hintLineColor ??=
  ...
  void _drawPointDisplayText(Canvas canvas) {
    if (_linePointPositions.isNotEmpty) {
      for (int lineIndex = 0;
          ...
          lineIndex++) {
        SantoPointsLine item = lines[lineIndex];
