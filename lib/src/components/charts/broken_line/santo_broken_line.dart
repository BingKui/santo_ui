...
/// 支持数据过多时左右滑动查看数据
/// 支持 X、Y 轴自定义展示文本内容
class SantoBrokenLine extends StatefulWidget {
  /// 表格宽高
  final Size size;

  /// 绘制线条的参数内容
  final List<SantoPointsLine> lines;
  ...
  /// y轴左侧刻度显示，不传则没有
  final List<SantoDialItem>? yDialValues;

  /// x 轴刻度
  final List<SantoDialItem>? xDialValues;
  ...
  @override
  State<StatefulWidget> createState() => SantoBrokenLineState();
}

class SantoBrokenLineState extends State<SantoBrokenLine> {
  int pointSelectIndex = -1;
  int lineSelectIndex = -1;

  @override
  Widget build(BuildContext context) {
    var _lineWithXPainter = SantoLinePainter(
      widget.lines,
    ...
    var yPainter = SantoLineYPainter(
      widget.lines,
  ...
  Widget _buildTouchTipWidget(SantoPointData pointData) {
    Widget touchTipWidget;
    SantoLineTouchData? selectLinePoint = pointData.lineTouchData;
