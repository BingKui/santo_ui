...
/// SantoMultiColumnPicker相关工具类
class SantoMultiColumnPickerUtil {
  /// 筛选项最多不超过三层,故直接写代码判断,本质为深度优先搜索。
  static int getTotalColumnCount(SantoPickerEntity? entity) {
    int count = 0;
    SantoPickerEntity? rootEntity = entity;
    ...
    if (rootEntity != null && rootEntity.children.isNotEmpty) {
      ...
      for (SantoPickerEntity firstLevelEntity in rootEntity.children) {
        if (firstLevelEntity.children.isNotEmpty) {
          count = count > 2 ? count : 2;
          for (SantoPickerEntity secondLevelEntity in firstLevelEntity.children) {
  ...
  /// 确定当前 Item 在第几层级
  static int getCurrentColumnIndex(SantoPickerEntity? currentItem) {
    int listIndex = -1;
    if (currentItem != null) {
      listIndex = 0;
  ...
  static bool isSelectedCountExceed(SantoPickerEntity? entity) {
