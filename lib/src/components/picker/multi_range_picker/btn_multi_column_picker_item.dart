...
class SantoMultiRangePickerCommonItem extends StatelessWidget {
  ...
  final ValueChanged<SantoPickerEntity>? itemSelectFunction;
  ...
  String _getSelectedItemCount(SantoPickerEntity item) {
    String itemCount = "";
    if ((SantoMultiColumnPickerUtil.getTotalColumnCount(item) < 3 ||
        !isFirstLevel)) {
      int count =
  ...
  bool _hasCheckBoxBrother(SantoPickerEntity item) {
    int? count;
    if (item.parent != null) {
