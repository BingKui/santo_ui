import 'package:santo_ui/src/components/picker/base/santo_picker.dart';
...
typedef SantoMultiDataPickerCreateWidgetCallback = Widget Function(
...
/// 数据适配 Delegate
abstract class SantoMultiDataPickerDelegate {
  /// 定义显示几列内容
...
class SantoMultiDataPicker extends StatefulWidget {
  /// 多级数据选择弹窗标题
  final String title;
  ...
  final SantoMultiDataPickerDelegate delegate;
  ...
  final SantoMultiDataPickerCreateWidgetCallback? createItemWidget;
  ...
  SantoMultiDataPicker(
    ...
    this.themeData = this.themeData!.merge(SantoPickerConfig(
      cancelTextStyle: SantoTextStyle.withStyle(cancelTextStyle),
      confirmTextStyle: SantoTextStyle.withStyle(confirmTextStyle),
      titleTextStyle: SantoTextStyle.withStyle(titleTextStyle),
      itemTextStyle: SantoTextStyle(color: textColor, fontSize: textFontSize),
      itemTextSelectedStyle:
      SantoTextStyle(color: textSelectedColor, fontSize: textFontSize),
    ));
  }

  @override
  _SantoMultiDataPickerState createState() => _SantoMultiDataPickerState();
...
class _SantoMultiDataPickerState extends State<SantoMultiDataPicker> {
...
/// 实现了部分默认逻辑的 Delegate
class SantoDefaultMultiDataPickerDelegate implements SantoMultiDataPickerDelegate {
  ///数据源
  List<SantoMultiDataPickerEntity> data;
  ...
  SantoDefaultMultiDataPickerDelegate(
      {required this.data,
    ...
    if (data.isNotEmpty) {
      ...
      for (SantoMultiDataPickerEntity santoPickerItem in data) {
        if (santoPickerItem.children.isNotEmpty) {
          _numberOfComponent = 2;

          for (SantoMultiDataPickerEntity santoPickerItem1 in santoPickerItem.children) {
            if (santoPickerItem1.children.isNotEmpty) {
  ...
  int numberOfRowsInComponent(int component) {
    ...
    if (0 == component) {
      ...
      List<SantoMultiDataPickerEntity> secondMap = data[firstSelectedIndex].children;
  ...
  String titleForRowInComponent(int component, int index) {
    if (0 == component) {
      ...
      SantoMultiDataPickerEntity santoPickerItem = data[firstSelectedIndex];
      List<SantoMultiDataPickerEntity> secondList = santoPickerItem.children;
      return secondList[index].text;
    } else {
      SantoMultiDataPickerEntity santoPickerItem = data[firstSelectedIndex];
      List<SantoMultiDataPickerEntity> secondList = santoPickerItem.children;
      List<SantoMultiDataPickerEntity> threeList = secondList[secondSelectedIndex].children;
      return threeList[index].text;
    }
  }
}

/// 适用于 SantoDefaultMultiDataPickerDelegate 的数据类
class SantoMultiDataPickerEntity {
  /// 显示内容
  final String text;
  ...
  /// 子项
  final List<SantoMultiDataPickerEntity> children;

  SantoMultiDataPickerEntity({
    required this.text,
    this.value,
    this.children = const [],
  });
