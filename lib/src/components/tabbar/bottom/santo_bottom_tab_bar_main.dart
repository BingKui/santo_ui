...
import 'package:santo_ui/src/components/tabbar/bottom/santo_bottom_tab_bar_item.dart';
...
/// tabBar显示状态
enum SantoBottomTabBarDisplayType {
  /// 固定显示状态
  fixed,
...
/// 简述：底部导航栏组件容器，其中[SantoBottomTabBarItem]控制每个tab的状态
/// 功能：底部导航栏主容器控制，如点击动画，每个tab的悬浮样式
/// 特别注意：默认关闭点击动画，为固定显示状态
class SantoBottomTabBar extends StatefulWidget {
  SantoBottomTabBar({
    Key? key,
    required this.items,
    this.onTap,
    this.currentIndex = 0,
    this.type = SantoBottomTabBarDisplayType.fixed,
    this.fixedColor,
    this.iconSize = 24.0,
  ...
  final List<SantoBottomTabBarItem> items;
...
/// 底部导航栏中状态控制类
class _BottomTabBarState extends State<SantoBottomTabBar> with TickerProviderStateMixin {
  ...
  /// 更新整个widget
  @override
  void didUpdateWidget(SantoBottomTabBar oldWidget) {
    ...
    if (widget.currentIndex != oldWidget.currentIndex) {
      switch (widget.type) {
        case SantoBottomTabBarDisplayType.fixed:
          break;
        case SantoBottomTabBarDisplayType.shifting:
...
class _BottomNavigationTile extends StatelessWidget {
  ...
  final SantoBottomTabBarDisplayType type;
  final SantoBottomTabBarItem item;
  ...
  Widget _buildIcon() {
    ...
    switch (type) {
      ...
      case SantoBottomTabBarDisplayType.shifting:
        tweenStart = 16.0;
        iconColor = selected ? SantoThemeConfigurator.instance.getConfig().commonConfig.brandPrimary : null;
        break;
    }
    return Align(
    ...
    return Align(
      ...
      child: Container(
        ...
        child: DefaultTextStyle.merge(
          style: TextStyle(
            fontSize: _kActiveFontSize,
            color: selected ? SantoThemeConfigurator.instance.getConfig().commonConfig.brandPrimary
