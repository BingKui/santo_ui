...
typedef SantoSearchBarLeadClickCallback = Function(
...
/// updateTextEdit 是暴漏给使用者的更新方法，该方法在组件的实现setState。
/// 比如想要刷新搜索框 就可以直接调用updateTextEdit()
typedef SantoSearchBarDismissClickCallback = Function(
    TextEditingController textEditingController, VoidCallback updateTextEdit);

/// 输入框输入变化的监听
typedef SantoSearchBarInputChangeCallback = Function(String input);

/// 输入框提交的监听
typedef SantoSearchBarInputSubmitCallback = Function(String input);

/// 用于搜索的AppBar
/// 该组件是[SantoAppBar]的特例包装，
/// 实现的思路是：将[SantoAppBar.title]设置为textField
/// 更多信息 请查看[SantoAppBar]
//ignore: must_be_immutable
class SantoSearchAppbar extends PreferredSize {
  /// 搜索框的文本输入控制器
  ...
  /// 搜索框的左侧leading
  final SantoSearchBarLeadClickCallback? leadClickCallback;

  /// 可以是字符串也可以是widget
  final dynamic leading;

  /// 取消点击的回调
  final SantoSearchBarDismissClickCallback? dismissClickCallback;

  /// 输入变化的监听
  final SantoSearchBarInputChangeCallback? searchBarInputChangeCallback;

  /// 输入框提交的监听
  final SantoSearchBarInputSubmitCallback? searchBarInputSubmitCallback;
  ...
  SantoSearchAppbar(
    ...
    this.themeData ??= SantoAppBarConfig.dark();
  ...
  Widget _createSearchChild(SantoAppBarConfig themeData) {
    return Row(
                ...
                child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: widget.hint ??
                          SantoIntl.of(context).localizedResource.inputSearchTip,
                    ),
