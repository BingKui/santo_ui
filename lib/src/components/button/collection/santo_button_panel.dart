...
class SantoButtonPanel extends StatefulWidget {
  /// 主按钮的文案
  ...
  final List<SantoButtonPanelConfig>? secondaryButtonList;
  ...
  @override
  _SantoButtonPanelState createState() => _SantoButtonPanelState();
}

class _SantoButtonPanelState extends State<SantoButtonPanel> {
  late GlobalKey _popWindowKey;

  List<SantoButtonPanelConfig> _secondaryButtonList = [];
  ...
  @override
  void didUpdateWidget(SantoButtonPanel oldWidget) {
    _initSecondaryButton();
  ...
  void _initSecondaryButton() {
    ...
    if (widget.secondaryButtonList?.isNotEmpty ?? false) {
      ...
      widget.secondaryButtonNameList!.forEach((name) {
        _secondaryButtonList
            .add(SantoButtonPanelConfig(name: name, isEnable: true));
      });
    }
  }
    ...
    if (_secondaryButtonList.length > 2) {
      ...
      return SantoPressFeedback(
        child: Container(
          ...
          height: 32,
          child: SantoTools.getAssetImage(SantoAsset.iconMore),
        ),
        key: _popWindowKey,
        onTap: () {
          SantoPopupListWindow.showButtonPanelPopList(context, _popWindowKey,
              data: list,
...
class SantoButtonPanelConfig {
  ...
  /// create SantoButtonPanelConfig
  const SantoButtonPanelConfig({
    required this.name,
    this.isEnable = true,
  });
}
