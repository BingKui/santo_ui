...
///
/// 和[SantoPairInfoTable]组件不同，该组件的每个组件都是单行展示
///
/// 除了基本的信息展示外，使用SantoRichGridInfo还可以实现富文本、复杂Widget的功能。
/// 样式：
///    支持文本、富文本和自定义的widget
///    常用的情况可以通过类中的静态函数构造
///    富文本和Icon的情况推荐使用 [SantoRichTextGenerator] 构造
///
/// SantoRichInfoGridWidget(
///     pairInfoList: <SantoRichGridInfo>[
///       SantoRichGridInfo.valueLastClickInfo('名称名称名称名称名称名称名称', '内容内容',
...
///  * [SantoPairInfoTable], 单列key-value信息集合组件
///
class SantoRichInfoGrid extends StatelessWidget {

  /// 待展示的文本信息
  final List<SantoRichGridInfo>? pairInfoList;
  ...
  /// the theme config of SantoRichInfoGrid
  final SantoPairRichInfoGridConfig? themeData;

  /// create SantoRichInfoGrid
  SantoRichInfoGrid({
    Key? key,
    this.pairInfoList,
    this.padding,
  ...
  Widget _buildGridView(context) {
    SantoPairRichInfoGridConfig defaultConfig =
        themeData ?? SantoPairRichInfoGridConfig();

    defaultConfig = defaultConfig.merge(SantoPairRichInfoGridConfig(
  ...
  Widget _getKeyWidget(SantoRichGridInfo info, double width, BuildContext context,
      SantoPairRichInfoGridConfig config) {
  ...
  Widget _getValueWidget(
      SantoRichGridInfo info, SantoPairRichInfoGridConfig config) {
...
/// 用于构建文本信息
class SantoRichGridInfo {

  ///
  final dynamic keyPart;
  final dynamic valuePart;

  SantoRichGridInfo(this.keyPart, this.valuePart);
  ...
  /// isArrow 是否最右侧存在箭头
  static SantoRichGridInfo valueLastClickInfo(
    ...
    SantoPairRichInfoGridConfig? themeData,
  }) {
    themeData ??= SantoPairRichInfoGridConfig();
    themeData = SantoThemeConfigurator.instance
        ...
        .pairRichInfoGridConfig
        .merge(themeData);
    themeData = themeData.merge(SantoPairRichInfoGridConfig(
        linkTextStyle: SantoTextStyle(color: clickColor)));
    ...
    Widget _getClickValue({required SantoPairRichInfoGridConfig themeData}) {
      return GestureDetector(
        onTap: () {
    ...
    return SantoRichGridInfo(key, value);
  }
}

TextStyle? _getKeyStyle({SantoPairRichInfoGridConfig? themeData}) =>
