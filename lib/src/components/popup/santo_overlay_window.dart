...
enum SantoOverlayPopDirection { none, top, bottom, left, right }

/// * 描述: Overlay 工具类。
class SantoOverlayWindow extends StatefulWidget {
  ...
  final SantoOverlayPopDirection popDirection;

  /// 要展示的内容
  final Widget content;

  const SantoOverlayWindow({
    required this.context,
    required this.targetKey,
    this.popDirection = SantoOverlayPopDirection.bottom,
    this.content = const SizedBox.shrink(),
  });

  @override
  State<StatefulWidget> createState() {
    return _SantoOverlayWindowState();
  }

  /// SantoOverlayWindow 工具方发，用于快速弹出 Overlay，
  /// 返回 [SantoOverlayController] 用于控制 Overlaywindow 的隐藏
  ...
  static SantoOverlayController? showOverlayWindow(
      BuildContext context, GlobalKey? targetKey,
      {Widget? content,
      SantoOverlayPopDirection popDirection = SantoOverlayPopDirection.bottom,
    ...
    overlayController = SantoOverlayController._(context, entry)..showOverlay();
    return overlayController;
  }
}

class _SantoOverlayWindowState extends State<SantoOverlayWindow> {
  /// targetView的位置
  Rect? _showRect;
  ...
  Widget _buildContent(Rect showRect) {
    ...
    if (widget.popDirection == SantoOverlayPopDirection.left) {
      realContent = Row(
    ...
    } else if (widget.popDirection == SantoOverlayPopDirection.right) {
      realContent = Row(
        mainAxisSize: MainAxisSize.max,
        children: <Widget>[
          Expanded(
    ...
    } else if (widget.popDirection == SantoOverlayPopDirection.top) {
      realContent = Column(
        mainAxisSize: MainAxisSize.max,
        children: <Widget>[
          Expanded(
  ...
  void _calculateOffset(Rect showRect) {
    ...
    } else if (widget.popDirection == SantoOverlayPopDirection.bottom) {
...
class SantoOverlayController {
  ...
  SantoOverlayController._(this.context, this._entry);
