...
class SantoRichTextGenerator {
  SantoRichTextGenerator();
  ...
  SantoRichTextGenerator addTextWithLink(
    String text, {
    String? url,
    TextStyle? textStyle,
    Color? linkColor,
  ...
  /// [fontSize] 文案大小 默认是16
  /// [color] 文案的颜色 默认是深黑色
  SantoRichTextGenerator addText(
    String text, {
    TextStyle? textStyle,
    double? fontSize,
    Color? color,
  ...
  SantoRichTextGenerator addIcon(
    Widget? icon, {
  ...
  /// 设置最多文案显示几行 默认是100行
  SantoRichTextGenerator setMaxLines(int maxLine) {
    if (maxLine > 0) {
      _maxLine = maxLine;
    }
    return this;
  ...
  /// 设置文案溢出的样式
  SantoRichTextGenerator setTextOverflow(TextOverflow overflow) {
    _overflow = overflow;
    return this;
  }
