...
相关组件 [SantoFlatSelectionWidget](../widgets/santo-flat-selection)

相关组件 [SantoSimpleSelection](../widgets/santo-simple-selection)

## 一、效果总览

![SantoSelectionViewIntro](./img/SantoSelectionViewIntro.png)
...
- 如果需要在回调中变更 entity 对应的筛选项数据，需要调用 entity.configChild() 方法。来重新构建筛选结点树。 例如：ConfigRowCountFunction、SantoCustomSelectionActionHandle、JoinTitleActionHandle、SantoOnWindowPreShow 等回调方法。
...
<img src="./img/SantoSelectionViewIntro2.png" style="zoom:50%;" />

- 筛选数据的结构为一个树结构

  ![](./img/SantoSelectionViewIntro3.png)
...
| -------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------ | ---------- |
| originalSelectionData      | `List<**SantoSelectionEntity**>`                                 | 筛选所需数据用于展示筛选菜单栏，以及展开的筛选弹窗。         | 是           | 空         |
| selectionViewController    | SantoSelectionViewController?                                  | 用于主动控制筛选弹窗的关闭和刷新菜单的内容状态               | 否           |            |
| onSelectionChanged         | `void Function(int menuIndex, Map<String, String> selectedParams, Map<String, String> customParams, SantoSetCustomSelectionMenuTitle setCustomMenuTitle)` | 点击筛选弹窗中的【确认】按钮时回调给外部选中的参数,包括:<br />menuIndex：当前展开的 menu 位置;<br />selectedParams 所有筛选弹窗中选中的参数；<br />customParams：拦截 CustomHandle 类型的筛选并设置的自定义参数；setCustomMenuTitle：对当前的筛选 Menu 设置 title 和高亮的回调函数，一般与 customParams 、**onCustomSelectionMenuClick**搭配使用，例如，通过 onCustomSelectionMenuClick 回调返回 customParams 参数，setCustomMenuTitle 根据 customParams 参数判断是否高亮，是否需要根据参数更新筛选菜单的 Title。 | 是           |            |
| configRowCount             | `SantoConfigTagCountPerRow(int index, SantoSelectionEntity entity)?` | 仅适用于类型为 SelectionWindowType.Range 的筛选弹窗，用于对 SelectionWindowType.Range 类型的列数做配置，回调参数为 menu 的所在位置 index 和 menu 的数据。 | 否           | 空         |
...
| onCustomSelectionMenuClick | `Function(int index, SantoSelectionEntity customMenuItem, SantoSetCustomSelectionParams customSelectionParams)?` | 自定义（CustomHandle）类型的 menu 被点击的回调。<br />int index: 自定义 menu 所在位置 SantoSelectionEntity customMenuItem 自定义 menu 的数据<br />customSelectionParams：设置自定义参数 | 否           |            |
| onCustomFloatingLayerClick | `Function(int index, SantoSelectionEntity customFloatingLayerEntity, SantoSetCustomFloatingLayerSelectionParams setCustomFloatingLayerSelectionParams)?` | 用于【**更多-筛选**】中自定义二级浮层的点击回调（自定义二级浮层类型为 customfloatinglayer） | 否           |            |
| onMoreSelectionMenuClick   | `void Function(int index, SantoOpenMorePage openMorePage)?`    | 点击【更多】筛选项时的回调。<br />index 为点击的位置<br />openMorePage：让用户触发的回调，用于展开更多筛选页面。`OpenMorePage = void Function({bool updateData, List moreSelections});` | 否           |            |
| onDefaultParamsPrepared    | `void Function(Map<String, String> selectedParams)?`         | 数据初始化完成之后返回默认选中参数的回调，在 SantoSelectioinViewState **initState** 中回触发回调<br />Map<String, String> selectedParams: 所有筛选菜单中的默认选中参数 | 否           |            |
| onSelectionPreShow         | `SelectionWindowType?` </br>`SelectionWindowType Function(int index, SantoSelectionEntity entity);` | 筛选弹窗打开前的回调方法, 并返回 SelectionWindowType 指定展示类型，是 Range 类型还是 List 类型。调用时机早于 onMenuItemClick<br />int index:点击筛选菜单的位置。<br />SantoSelectionEntity entity: index 菜单位置的筛选数据。 | 否           |            |
...
class SantoSelectionEntity {
  ...
  /// 父级筛选项
  SantoSelectionEntity parent;

  /// 筛选类型，具体参见 [SantoSelectionFilterType]
  SantoSelectionFilterType filterType;

  /// 筛选弹窗展示风格对应的首字母小写的字符串，例如 `range`、`list`，参见 [SantoSelectionWindowType]
  String showType;

  /// 筛选弹窗展示风格，具体参见 [SantoSelectionWindowType]
  SantoSelectionWindowType filterShowType;
...
SantoMoreSelectionPage，一般由 SantoSelectionView 的 onMoreSelectionMenuClick 触发跳转

<img src="./img/SantoSelectionViewMorePage.png" style="zoom: 25%;" />
...
SantoLayerMoreSelectionPage，一般由 SantoMoreSelectionPage 触发跳转

<img src="./img/SantoSelectionViewMorePage2.png" style="zoom:20%;" />

```dart
SantoLayerMoreSelectionPage({this.entityData, this.themeData});
```
...
[示例数据 multi_list_filter.json](https://santo-ui.example.com/widgets/selection/santo-selection-view/multi_list_filter.json)

<img src="./img/SantoSelectionViewDemo1.png" alt="image-20211031102133506" style="zoom:50%;" />
...
[示例数据 multi_range_filter.json](https://santo-ui.example.com/widgets/selection/santo-selection-view/multi_range_filter.json)

<img src="./img/SantoSelectionViewDemo2.png" alt="image-20211031103127927" style="zoom:50%;" />
...
[示例数据 more_filter.json](https://santo-ui.example.com/widgets/selection/santo-selection-view/more_filter.json)

 <img src="./img/SantoSelectionViewMorePage.png" alt="image-20211031104023886" style="zoom: 25%;" />
...
[示例数据 customhandle_filter.json](https://santo-ui.example.com/widgets/selection/santo-selection-view/customhandle_filter.json)

![customhandle_filter](./img/SantoSelectionViewDemo4.gif)
...
SantoSelectionView(
  ...
  onSelectionChanged: (int menuIndex,
      ...
      setCustomTitleFunction(
          menuTitle:
              SantoTools.isEmpty(customParams) ? "" : customParams['CKey'] ?? "",
...
[示例数据 date_range_filter.json](https://santo-ui.example.com/widgets/selection/santo-selection-view/date_range_filter.json)

![](./img/SantoSelectionViewDemo5.png)
