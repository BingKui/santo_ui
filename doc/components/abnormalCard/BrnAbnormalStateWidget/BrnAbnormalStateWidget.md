---
title: SantoAbnormalStateWidget
group:
  title: Empty
  order: 11
---
...
| action           | `SantoEmptyStatusIndexedActionClickCallback? = void Function(int index)index?` | 点击回调，在 SingleButton 类型或者 enablePageTap = true 点击空白区域时返回 0；其他根据点击的位置返回。 | 否           |                            |
...
| themeData        | SantoAbnormalStateConfig?                                      | 主题配置                                                     | 否           | 无                         |
...
SantoAbnormalStateWidget(
      ...
      operateTexts: ["切换账号"],
      action: (_) {
        SantoToast.show("第$_个按钮被点击了", context);
      },
);
```
