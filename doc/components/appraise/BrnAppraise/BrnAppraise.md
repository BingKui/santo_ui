...
![](./img/SantoAppraiseDemoIntro.png)
...
1. SantoAppraise：评价组件，支持表情包和五角星两种，可使用在页面和弹窗里。
2. [SantoAppraiseBottomPicker](../widgets/santo-appraise-bottom-picker)：评价组件底部弹窗，对 SantoAppraise 做了一层 picker 的封装，参数与 SantoAppraise 完全一致。
...
| iconDescriptions | `List<String>?`           | 点击表情时对应等级的提示文案。若 `type=SantoAppraiseType.Emoji`，则 list 长度为 5，不足 5 个时请在对应位置补空字符串。若 `type=SantoAppraiseType.Star`，list 长度不能比传入的 SantoAppraiseConfig 中的 count 小。 | 否           | 默认值为国际化配置的数组，['不好','还行','满意','很棒','超惊喜'] |
| tags             | `List<String>?`          | 供选择的标签数据                                             | 否           | 无                                     |
| inputHintText    | String                   | 输入框的提示文字                                             | 否           | ''                                     |
| onConfirm        | SantoAppraiseConfirmClick? | 点击提交时的回调，其中 index 是选中的表情或者五角星的 index，selectedTags 是选中的标签，input 是输入框的内容 | 否           | 无                                     |
| config           | SantoAppraiseConfig        | 配置类，具体见下边其他参数说明                               | 否           | SantoAppraiseConfig()                    |
...
| headerPadding           | EdgeInsets?                    | header 部分的 padding，只有居中样式支持设置                  | 否           | `headerType=SantoAppraiseHeader.center`时，为 `EdgeInsets.only(top: 20, bottom: 20)`，其余为 `EdgeInsets.only(left: 20, top: 16, right: 16, bottom: 20)` |
| titleMaxLines           | int                            | 标题的最大行数                                               | 否           | 1                                                            |
| onCancel                | SantoAppraiseCloseClickCallBack? | 点击关闭按钮时的回调                                         | 否           | 无                                                           |
| count                   | int                            | `type=SantoAppraiseType.Star`时所需五角星的个数                | 否           | 5                                                            |
| iconDescriptions        | `List<String>`                 | 点击表情时的文案。`type=SantoAppraiseType.Star`时，该列表的数据个数不能比 count 少。`type=SantoAppraiseType.Emoji`时，list 长度应为 5，不足 5 个时请在对应位置补空字符串 | 否           | ['不好', '还行', '满意', '很棒', '超惊喜']                   |
...
| iconClickCallback       | SantoAppraiseIconClick?          | 点击打分时的回调                                             | 否           | 无                                                           |
| inputTextChangeCallback | SantoInputTextChangeCallback?    | 输入框内容改变的回调                                         | 否           | 无                                                           |
| tagSelectCallback       | SantoAppraiseTagClick?           | 选择标签时的回调                                             | 否           | 无                                                           |

```dart
const SantoAppraiseConfig({
...
![](./img/SantoAppraiseDemo2.png) ![](./img/SantoAppraiseDemo2_1.png)
