...
![](./img/SantoMultiDataPickerIntro.png)
...
| delegate | SantoMultiDataPickerDelegate | 滚动组件每一列、每一行的具体内容设置，自定义delegate继承这个类，实现具体方法。 | 是 |  |
...
| createItemWidget | SantoMultiDataPickerCreateWidgetCallback? | 返回自定义itemwidget | 否 |  |
| sync | bool | 是否复位数据位置 | 否 | true |
| themeData | SantoPickerConfig? | Picker配置类，配置详情见SantoPickerConfig | 否 |  |
...
注意示例代码中，Santo1RowDelegate、Santo2RowDelegate 需要自行实现。参考 Demo 代码。

### 效果1：单列滚动选择

![](./img/SantoMultiDataPickerDemo1.png)
...
![](./img/SantoMultiDataPickerDemo2.png)

```dart
SantoMultiDataPicker(
  context: context,
  title: '委托来源',
  delegate: Santo2RowDelegate(firstSelectedIndex: 1, secondSelectedIndex: 0),
  confirmClick: (list) {
    SantoToast.show(list.toString(), context);
  },
).show();
...
![](./img/SantoMultiDataPickerDemo3.png)
...
SantoMultiDataPicker(
  ...
  context: context,
  title: '委托来源',
  delegate: Santo2RowCustomDelegate(firstSelectedIndex: 1, secondSelectedIndex: 0),
  confirmClick: (list) {
    SantoToast.show(list.toString(), context);
  },
).show();
...
class Santo2RowCustomDelegate implements SantoMultiDataPickerDelegate {
  ...
  Santo2RowCustomDelegate({this.firstSelectedIndex = 0, this.secondSelectedIndex = 0});
...
![](./img/SantoMultiDataPickerDemo4.png)
