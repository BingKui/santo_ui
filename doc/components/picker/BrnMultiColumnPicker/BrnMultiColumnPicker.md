...
![](./img/SantoMultiColumnPickerIntro.png)
...
1. 所有节点 SantoPickerEntity 的 type 字段必须有具体类型，不可为空；目前支持类型有 radio、checkbox。
...
| ------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | -------- | ---------------------------- |
| entity                    | SantoPickerEntity                                              | 筛选数据源                                                   | 是       |                              |
| maxHeight                 | double                                                       | Picker 展示最大高度                                          | 否       | 280.0                        |
| showSelectedCount         | bool                                                         | 展示选中的条目的个数                                         | 否       | false                        |
| onConfirm                 | `void SantoOnPickerConfirm(Map<String, List<SantoPickerEntity>> results,int? firstIndex, int? secondIndex, int? thirdIndex)?` | 选择数据后回调函数                                           | 否       | null                         |
| onEntityTap               | `void SantoOnEntityTap(int columnIndex, int rowIndex, SantoPickerEntity entity)?` | 选择项目后回调函数                                           | 否       | null                         |
| pickerTitleConfig         | SantoPickerTitleConfig                                         | Picker 标题数据配置                                          | 否       | SantoPickerTitleConfig.Default |
| isIncludeUnLimit          | bool                                                         | 当选中”不限“的时候，返回的 reslut 结果是否包含 “不限” 选项，“不限” 选项 value 字段不能为空，上一级 key 字段不能为空 | 否       | false                        |
| canSelectEntryInterceptor | `bool SantoOnSelectEntityInterceptor( int listIndex, int index, SantoPickerEntity entity)?` | 当前选项是否可以被选中:返回 true 可以被选中 false 不可以被选中 | 否       |                              |
| defaultFocusedIndexes     | `List<int>?`                                                 | 初始化时的选中选项                                           | 否       |                              |
| themeData                 | SantoPickerConfig?                                             | 主题定制，只有 Picker Title 部分样式生效                     | 否       |                              |
...
[示例数据 list_picker.json](https://santo-ui.example.com/widgets/picker/santo-multi-column-picker/list_picker.json)

<img src="./img/SantoMultiColumnPickerDemo.png" alt="image-20211115110957518" style="zoom: 33%;" />

```dart
/// 加载 Picker 数据
rootBundle.loadString('assets/list_picker.json').then((data) {
  List<SantoPickerEntity> _selectionData = List()
...
/// 弹出 Picker
void _showRangePicker(BuildContext context, List<SantoPickerEntity> _selectionData) {
  _selectionData?.forEach((f) => f.configRelationshipAndDefaultValue());
