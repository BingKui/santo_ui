---
title: SantoSelectTagsWithInputPicker
group:
  title: Picker
  order: 23
---

# SantoSelectTagsWithInputPicker

可作为当操作“驳回/无效”等场景时用来填写原因的简单录入。

## 一、效果总览

<img src="./img/SantoSelectTagsWithInputPicker1.png" style="zoom:50%;" />
...
| confirm | SantoTagInputConfirmClickCallback? | 点击“确定”按钮事件回调 | 否 | - |
| cancelCallBack | SantoTagInputCancelClickCallBack? | 关闭Picker事件回调 | 否 | - |
...
| tagPickerConfig | SantoTagsInputPickerConfig | 标签数据源 | 是 | null |
...
#### SantoTagInputConfirmClickCallback
...
```
#### SantoTagInputCancelClickCallBack
...
List<SantoTagInputItemBean> items = List();
for (int i = 0; i < tags.length; i++) {
  String it = tags[i];
  SantoTagInputItemBean item = SantoTagInputItemBean(name: it, index: i, needExplane: (i % 2 == 0));
  items.add(item);
}
