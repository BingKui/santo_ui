...
# SantoGallerySummaryPage

查看大图交互模式-列表页

相关组件 [SantoGalleryDetailPage](../widgets/santo-gallery-detail-page)

## 一、效果总览

<img src="./img/SantoGallerySummaryPageIntro.png" style="zoom:50%;" /> 
...
2. 查看图片，查看视频，pdf(自行扩展配置接口**SantoBasicItemConfig**)

### **注意事项**

在图片数量较多时，可能引起OOM，可以自行继承**SantoPhotoItemConfig，**将其中的图片组件替换为架构组提供的图片组件。
...
| controller | SantoGalleryController | 控制图片查看刷新 | 否 | 无 |
...
class SantoPhotoGroupConfig extends SantoBasicGroupConfig {  
  ...
  //通过url列表生成配置
  SantoPhotoGroupConfig.url({this.title, @required this.urls, this.themeData}) {
    configList = List();
    urls.forEach((item) => configList.add(SantoPhotoItemConfig(url: item, themeData: themeData)));
  }

  //自定义配置列表
  SantoPhotoGroupConfig({this.urls, this.title, this.configList, this.themeData});
}
```
...
| themeData           | SantoGalleryDetailConfig | 主题配置                                                     | 否           | 全局主题配置                  |
...
通过SantoPhotoItemConfig定制每一个Item的配置 ，优点：可以对图片的每一个item进行配置，支持传入更多信息，但配置稍微麻烦

<img src="./img/SantoGallerySummaryPageDemo.png" alt="image-20211101120850446" style="zoom: 33%;" /> 
