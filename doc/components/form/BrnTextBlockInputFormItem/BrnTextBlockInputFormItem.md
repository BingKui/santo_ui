---
title: SantoTextBlockInputFormItem
group:
  title: Form
  order: 12
---
...
![](./img/SantoTextBlockInputFormItemIntro.png)
...
| type | Stirng | 录入项类型，主要用于录入类型页面框架中 | 否 | SantoInputItemType.textBlockInputType | 外部可根据此字段判断表单项类型 |
...
| inputType | String? | 输入内容类型，指定键盘类型，参见 `SantoInputType` | 否 | 无 | 详见**SantoInputType**类，注意：无法通过指定键盘类型确保输入。比如不能通过指定数字键盘确保用户只输入数字。如果有要求用户只输入特定字符的需求请使用**inputFormatters**参数 |
...
| themeData | SantoFormItemConfig? | 表单主题配置 | 否 | 无 | |
...
![](./img/SantoTextBlockInputFormItemDemo1.png)
```dart
SantoTextBlockInputFormItem(
...
```
### 效果2：全功能样式
![](./img/SantoTextBlockInputFormItemDemo2.png)
...
![](./img/SantoTextBlockInputFormItemDemo3.png)
```dart
SantoTextBlockInputFormItem(
  controller: TextEditingController()..text = "hello",
  prefixIconType: SantoPrefixIconType.add,
...
![](./img/SantoTextBlockInputFormItemDemo4.png)
```dart
SantoTextBlockInputFormItem(
  controller: TextEditingController()..text = "hello",
  prefixIconType: SantoPrefixIconType.add,
