...
![image-20211031173803670](./img/SantoDeleteTagIntro.png) 
...
| controller | SantoDeleteTagController? | 标签控制器，用于主动添加标签，如果只需要删除标签并进行回调可以不传控制器 | 否 |  |
...
| themeData; | SantoTagConfig? | 主题配置信息，包含标签背景色、文本颜色等字段 | 否 |  |

### SantoDeleteTagController
...
![image-20211031174539748](./img/SantoDeleteTagDemo1.png) 
...
SantoDeleteTag(
  ...
  onTagDelete: (tags, tag, index) {
    SantoToast.show('剩余的标签为：${tags.toString()},删除了的标签为：$tag  ,删除的标签index为$index', context);
  },
)
...
![image-20211031174841310](./img/SantoDeleteTagDemo2.png) 
