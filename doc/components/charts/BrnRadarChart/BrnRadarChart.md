...
<img src="./img/SantoRadarChartIntro.png" style="zoom: 33%;" />
...
1. 默认风格，通过使用命名构造函数 SantoRadarChart.defautStyle()进行创建，创建出的雷达图将与设计规范完全一致。特点：使用者只需提供一些少量的基础参数，便能创建一个满足设计规范的雷达图。但是，使用者无法做一些定制化效果，没有点击效果，并且对提供的参数也会有一些限制。
...
使用 Santo 默认风格，通过使用命名构造函数 `SantoRadarChart.defautStyle()` 进行创建。
...
| provider        | SantoRadarChartDataProvider | 提供绘制雷达图所需要的数据                     | 使用默认构造方法必传。默认样式无此参数配置。         |                   |                                                              |
...
<img src="./img/SantoRadarChartDemo1.png" style="zoom:50%;" />
...
<img src="./img/SantoRadarChartDemo2.png" style="zoom:67%;" />
