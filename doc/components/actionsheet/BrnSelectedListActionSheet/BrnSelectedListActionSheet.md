---
title: SantoSelectedListActionSheet
group:
  title: ActionSheet
  order: 2
---
...
![](./img/SantoSelectedListActionSheetDemo1.png)
...
1. 通用的已选列表弹窗。包括顶部工具条和中间的数据列表两部分，不包括底部的按钮交互区域，底部的按钮Demo中使用的是 [SantoBottomButtonPanel](../widgets/santo-button-panel)。整体布局通过`OverlayEntry` 实现，将控制列表的控制功能和已选列表视图独立开来，一来可以更加方便的控制列表，二来可以充分自定义控制列表的功能区域。
2. 工具条：包括关闭弹窗和清空按钮。
...
| controller | `SantoSelectedItemListActionSheetWidgetController?` | 控制视图隐藏/刷新列表等方法 | 否 | 无 |
...
#### SantoSelectedListActionSheetController
...
SantoSelectedListActionSheetController controller = SantoSelectedListActionSheetController();
...
![](./img/SantoSelectedListActionSheetDemo2.png)


```dart
List _data = ['数据源1', '数据源2', '数据源3', '数据源4']; 
var _bottomActionKey = GlobalKey();  
controller = SantoSelectedItemListActionSheetWidgetController();  
                        ...
                        if (!controller.isHidden) {
                          ...
                          SantoSelectedListActionSheet<String>(
                                  ...
                                  onClear: () {
                                    controller.dismiss();
                                    // 自定义清空的操作，可以不实现，会走默认的清空操作。
                                    SantoDialogManager.showConfirmDialog(
                                        context,
                                        title: "确定要清空已选列表吗?",
                                        cancel: '取消',
                                        confirm: '确定', onConfirm: () {
...
![](./img/SantoSelectedListActionSheetDemo3.png)