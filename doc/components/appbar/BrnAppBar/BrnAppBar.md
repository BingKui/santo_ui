...
2. SantoAppBar即提供了默认的返回操作（maybePop），有支持自定义返回事件
3. 该类文件提供了常用的SantoTextAction、SantoIconAction、SantoBackLeading以及SantoDoubleLeading
...
| title | dynamic | AppBar中间显示的内容 | 否 | 必须是String或者Widget。为String时,会使用[SantoAppBarTitle]来加载title |
...
| themeData | SantoAppBarConfig? | SantoAppBar对应的主题定制类 | 否 | |
