...
| --- | --- | --- | --- | --- |
| searchController | SantoSearchTextController? | 用于控制 clear Icon 和右侧 Action 的显示与隐藏。等其他复杂的操作。 | 否 | 无 |
...
| textStyle | TextStyle? | 输入文本的样式 | 否 | TextStyle(textBaseline: TextBaseline.alphabetic,color: SantoColor.instance.F0Color, fontSize: 16) |
| prefixIcon | Widget? | 输入框前置的图标 | 否 | 默认为搜索图标 |
| onTextChange | `SantoOnSearchTextChange=void Function(String content)?` | 搜索框输入内容改变时候的回调函数,str为输入内容 | 否 | 无 |
| onTextCommit | `SantoOnCommit=void Function(String content)?` | 输入内容点击搜索后的回调，content为输入内容 | 否 | 无 |
| onTextClear | `SantoOnTextClear = bool Function()?` | 拦截取消的事件。<br />1、返回值为true，表明用户想要拦截，则不会走默认的清除行为<br />2、如果返回值为false，表明用户不想要拦截，在执行了用户的行为之后，还会走默认的行为 | 否 | 无 |
...
![S](./img/SantoSearchTextDemo1.gif) 
```dart
SantoSearchTextController searchController = SantoSearchTextController();  
...
### 效果2: 自定义Search 的边框颜色
 ...
 ![image-20211028175929118](./img/SantoSearchTextDemo2.png)
