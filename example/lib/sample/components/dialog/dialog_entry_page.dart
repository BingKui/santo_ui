...
class DialogEntryPage extends StatelessWidget {
    ...
    showDialog(
        ...
        builder: (_) => StatefulBuilder(
              builder: (context, state) {
                return SantoSingleSelectDialog(
                    isClose: true,
                    title: '请选择无效客源原因',
  ...
  void _showMiddleMultiSelectDialog(BuildContext context) {
    ...
    showDialog(
        context: context,
        builder: (_) => SantoMultiSelectDialog(
            title: "请您评价该条线索",
            isClose: true,
            conditions: data,
  ...
  void _showStyle1Dialog0(BuildContext context) {
    SantoDialogManager.showSingleButtonDialog(context, label: "知道了", message: "辅助内容容信息", onTap: () {
  ...
  void _showStyle5_1Dialog(BuildContext context) {
    SantoDialogManager.showMoreButtonDialog(context,
        actions: [
          '选项一',
          '选项二',
          '选项三',
  ...
  void _showStyle71Dialog(BuildContext context) {
    SantoDialogManager.showConfirmDialog(context,
        showIcon: true,
        iconWidget: SantoTools.getAssetImage("icons/icon_warning.png"),
  ...
  void _showStyle7Dialog(BuildContext context) {
    SantoDialogManager.showSingleButtonDialog(context, showIcon: true, title: "恭喜你完成填写", label: "确定",
        onTap: () {
      SantoToast.show("确定", context);
      Navigator.pop(context);
    });
  ...
  ///底部有输入框弹框
  void _showSantoLoadingDialog(BuildContext context) {
    SantoLoadingDialog.show(context).then((value) {
          SantoToast.show('result: $value', context);
        });
    Future.delayed(Duration(seconds: 5)).then((_) {
      SantoLoadingDialog.dismiss(context, 'dismiss 定时取消');
    });
  }

  ///安全关闭的弹框
  void _showSafeDialog(BuildContext context) {
    SantoSafeDialog.show(
        context: context,
        tag: "AA",
        builder: (context) {
      ...
      SantoToast.show('result: $result ', context);
    });
    ...
    Future.delayed(Duration(seconds: 5)).then((_) {
      SantoSafeDialog.dismiss(context: context, tag: "AA", result: 'delete dialog AA by tag AA');
    });

    Future.delayed(Duration(seconds: 10)).then((_) {
      SantoSafeDialog.dismiss(context: context, result: 'delete dialog BB by default tag');
    });
  }

  void _showSantoShareDialog3(BuildContext context) {
    SantoShareDialog santoShareDialog = new SantoShareDialog(
      context: context,
      shareChannels: [
      ...
      clickCallBack: (int channel, int index) {
        SantoToast.show("channel: $channel, index: $index", context);
      },
      getCustomChannelWidget: (int index) {
        if (index == 2)
          return SantoTools.getAssetImage("images/icon_custom_share.png");
        else
          return null;
      },
      // 自定义图标
    ...
    santoShareDialog.show();
  }

  void _showSantoShareDialog5(BuildContext context) {
    SantoShareDialog santoShareDialog = new SantoShareDialog(
      context: context,
      shareChannels: [
        SantoShareItemConstants.shareWeiXin,
  ...
  void _showSantoTwoVerticalButtonDialogDialog(BuildContext context) {
    SantoEnhanceOperationDialog santoShareDialog = new SantoEnhanceOperationDialog(
      context: context,
      iconType: SantoDialogConstants.iconAlert,
      titleText: "强提示文案",
  ...
  void _showSantoOneVerticalButtonDialogDialog(BuildContext context) {
    SantoEnhanceOperationDialog santoShareDialog = new SantoEnhanceOperationDialog(
      iconType: SantoDialogConstants.iconWarning,
      context: context,
      titleText: "强提示文案",
      descText: "这里是文案这里是文案这里是文案这里是文案这里是文案这里是文案这里是文案这里是文案",
      mainButtonText: "我知道了",
  ...
  void _showStyle82Dialog(BuildContext context) {
    showDialog(
        context: context,
        builder: (_) => SantoScrollableTextDialog(
            title: "纯文本标题",
            contentText:
